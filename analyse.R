# analyse.R: by Daan Gerla, d.gerla@overijssel.nl, daan.gerla@anteagroup.nl
...
reshape_intersection <- function(intersection) {
    c2 <- c("HABTYPE", "VEGTYPE", "PERC", "OPP", "ISHD", "KWAL", "OPM")
    c2 <- c(mapply(function(n) paste0(c2, n), 1:10))
    c1 <- names(intersection)[!(names(intersection) %in% c2)]
    d1 <- intersection[c1]     # the columns referring to gebied
    d2 <- intersection[c(c2)]  # the columns referring to deelgebieden
    f <- function(cols) {
...
doelbereik <- read_excel(paste0(root, '/data/doelbereik.xlsx'))
bepaal_bereik <- Vectorize(function(habtype, variabele, waarde, tabel=doelbereik) {
...
get_habveg <- function(eiv, hab) {
    # Randvoorwaarden per intersectie van habitattype en vegetatietype:
    intersection <- intersect_eiv_hab(eiv, hab)
    habveg <- reshape_intersection(intersection@data)
    habveg$HABTYPE_NAAM <- mapply(function(h) doelbereik$HABTYPE_NAAM[doelbereik$HABTYPE == h][1], habveg$HABTYPE)
...
get_randvoorwaarden <- function(habveg) {
    # Randvoorwaarden en bereik per van habitattype en vegetatietype:
...
hab_ <- simplify_habitat(intersection@data)
#NOTE: hab_ bevat nu extra rijen voor de habitats met gemengde habitattypen. 
# We moeten nog correct uitrekenen wat het oppervlakte is elk van de
# 'deelgebieden' (d.w.z. de oppervlaktes van elke habitattype binnen een 
# gemengd habitattype).
hab_$voedselrijkdom <- trofie2voedselrijkdom(hab_$Trofie)
hab_$zuurgraad <- pH2zuurgraad(hab_$pH)
hab_$vochttoestand <- GVG2vochttoestand(-hab_$GVG, -hab_$GLG) #NOTE: minus signs

head(hab_)
rows <- hab_$HABTYPE != 'H0000'
head(hab_[cols][rows, ], 55)

#ddply(hab_, .(N2K_GEBIED, HABTYPE), summarize, 
#      OPP=sum(OPP), 
...
(type_turboveg <- as.character(opname$habitat_type$ASSOCIA_02))
